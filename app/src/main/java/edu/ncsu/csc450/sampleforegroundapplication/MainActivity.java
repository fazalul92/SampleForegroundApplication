...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    void initConnection() {
        ...
        mContextCallback = new IContextCallback.Stub() {
            ...
            @Override
            public void notifyAtNewyorkCallback() throws RemoteException {
                Log.d("NYC Notification","Invoked");
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        nycText.setText("You are in New York City");
                    }
                });
            }
        };
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.nyButton: {
                try {
                    if(mContextIsBound) {
                        // Async call to service function. Response handled in callback function notifyAtNewyorkCallback() in initConnection()
                        contextService.notifyAtNewYork(mContextCallback);
                    }
                } catch (RemoteException e){
                    e.printStackTrace();
                }
